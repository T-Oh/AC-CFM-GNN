...
class HurricaneDataset(Dataset):
    ...
    def processed_file_names(self):
        ...
        if not files:
            return []
        if os.path.isdir(os.path.join(processed_dir, files[0])):
            return []
    ...
    def process(self):
        if self.data_type in ['AC', 'Zhu', 'Zhu_mat73', 'ANGF_Vcf', 'LSTM']:
            self.process_ac()
        #elif self.data_type =='LSTM':
        #    self.process_lstm()
    ...
    def process_ac(self):
        ...
        init_data, filetype = self.load_mat_file('data/raw/' + 'pwsdata.mat')
        ...
        for raw_path in self.raw_paths:
            ...
            if self.data_type == 'LSTM':
                data = Data(x=node_feature.float(), edge_index=adj, edge_attr=edge_attr, node_labels=node_labels, y=graph_label) 
                scenario_dir = os.path.join(self.root, f'/scenario_{scenario}/')
