...
# Handle multiple node labels
fig, axs = plt.subplots(node_label_bins.shape[0], 1, figsize=(10, 4 * node_label_bins.shape[0]))

for i in range(node_label_bins.shape[0]):
    axs[i].bar(node_label_bins[i], node_label_hists[i], width=(node_label_bins[i][1] - node_label_bins[i][0]), align='edge', color='red')
    axs[i].set_xlabel(f"Label {i} VOltage [p.u.]")
    axs[i].set_ylabel('Number of Nodes')

fig.savefig(path+"ac_node_label_distr_"+NAME+".png", bbox_inches='tight')
