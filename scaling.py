# -*- coding: utf-8 -*-
"""
Created on Tue Feb 14 12:40:18 2023
...
def get_feature_stds(x_means, edge_means,processed_dir):
    ...
    for file in os.listdir(processed_dir):
        ...
        for i in range(len(edge_attr)):
            ...
            edge_count += 1
    return np.sqrt(x_stds/node_count), npt.sqrt(edge_stds/edge_count)
...
x_stds, edge_stds = get_feature_stds(x_means, edge_attr_means, processed_dir)
...
for file in os.listdir(processed_dir):
    ...
    edge_attr[:,1] = (data['edge_attr'][:,1]-edge_attr_means[1])/edge_stds[1]
    edge_attr[:,2] = (data['edge_attr'][:,2]-edge_attr_means[2])/edge_stds[2]
    edge_attr[:,4] = (data['edge_attr'][:,4]-edge_attr_means[3])/edge_stds[3]
    #reactance
    ...
    data = Data(x=x, edge_index=data['edge_index'], edge_attr=edge_attr, node_labels=node_labels) 
