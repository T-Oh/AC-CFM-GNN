...
if cfg["study::run"]:
    ...
    #Loading GNN model
    model = get_model(cfg, params, num_features, num_edge_features, num_targets)   #TO get_model does not load an old model but create a new one 
    model.to(device)
    ...
    losses, final_eval, output, labels = run_training(trainloader, testloader, engine, epochs=cfg["epochs"])
    torch.save(list(output), "results/" + cfg["dataset::path"] + f"output.pt") #saving train losses
    torch.save(list(labels), "results/" + cfg["dataset::path"] + f"labels.pt") #saving train losses
    torch.save(list(losses), "results/" + cfg["dataset::path"] + "losses1.pt") #saving train losses
    plt.plot(losses)
