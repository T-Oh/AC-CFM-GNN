...
"""
also adds the graph_label to the data instances
"""
    ...
    for file in os.listdir(processed_dir):
        if file.startswith('data'):
            ...
            for i in range(len(node_labels)):
                ...
                graph_label += node_labels[i]
                node_labels_mean += node_labels[i]
                
            data = Data(x=torch.transpose(x,0,1).float(), edge_index=adj, edge_attr=torch.transpose(edge_attr,0,1), node_labels=node_labels, y=graph_label) 
